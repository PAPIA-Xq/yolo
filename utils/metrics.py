...
def bbox_inner_shape_iou(box1, box2, x1y1x2y2=True, eps=1e-7):
    """
    Calculate Inner-ShapeIoU (inner Shape-aware IoU).
    Focused on improving small object detection by considering shape information.

    Args:
        box1 (torch.Tensor): Predicted boxes (N, 4)
        box2 (torch.Tensor): Ground truth boxes (M, 4)
        x1y1x2y2 (bool): Input box format
        eps (float): Small value to avoid division by zero

    Returns:
        (torch.Tensor): Inner-ShapeIoU values
    """
    # Get coordinates
    if x1y1x2y2:
        b1_x1, b1_y1, b1_x2, b1_y2 = box1[0], box1[1], box1[2], box1[3]
    ...
    aspect1 = w1 / h1
    aspect2 = w2 / h2
    aspect_diff = torch.abs(aspect1 - aspect2) / (aspect1 + aspect2 + eps)

    # Inner overlap penalty (focus on how well prediction fits inside GT)
    inner_overlap = inter / (w2 * h2 + eps)  # Intersection over GT area

    # Combined Shape-IoU
    shape_penalty = 1 - inner_overlap + aspect_diff * 0.1
    shape_iou = iou - shape_penalty * 0.5

    return shape_iou.clamp(0, 1)
